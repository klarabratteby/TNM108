...
# Built-in function RFE specifying to select the 4 best features
best_features = 4
...
# Standard function CalcMeasures and divide the label vector Y from the features X
...
def CalcMeasures(method, y_pred, y_true, df_f1=df_f1, df_precision=df_precision, df_recall=df_recall):
    df_f1.loc[len(df_f1)] = [method] + \
        list(f1_score(y_true, y_pred, average=None, zero_division=1))
    df_precision.loc[len(df_precision)] = [method] + \
        list(precision_score(y_pred, y_true, average=None, zero_division=1))

    df_recall.loc[len(df_recall)] = [
        method]+list(recall_score(y_pred, y_true, average=None, zero_division=1))


X = df[df.columns[:-1]].values
Y = df[df.columns[-1]].values

# A 10 folds cross validation has been used
cv = 10
...
# Output: df_f1
df_f1 = df_f1.set_index('method')
print(df_f1)
# Output: df_precision
df_precision = df_precision.set_index('method')
print(df_precision)
# Output: df_recall
df_recall = df_recall.set_index('method')
print(df_recall)
